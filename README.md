...
GrabInk 在 Wayland 下通过标准的 [XDG Desktop Portal](https://flatpak.github.io/xdg-desktop-portal/docs/doc-org.freedesktop.portal.Screenshot.html) 获取画面，再在应用内完成区域选择、标注、保存和复制。Wayland 路径不依赖 X11 抓屏，也不绑定 GNOME Shell 私有接口。

![GrabInk 程序截图](https://img.cdn1.vip/i/6a52698f6b18b_1783785871.webp)
...
GrabInk 要解决的是这段缺失的工作流：**在遵守 Wayland 安全模型的前提下，把截图、框选和标注放回同一个流程中。**
...
Wayland 下，portal 负责向 GrabInk 交付截图。区域截图仍先获取完整屏幕，再由 GrabInk 完成框选和标注。窗口截图优先请求 portal v3 的 `Active Window` 目标；如果系统只提供窗口选择或旧版交互接口，会显示系统选择界面。首次使用或受沙箱策略影响时，系统也可能显示授权界面；这是 Wayland 安全模型的一部分，应用无法绕过。

### 跨平台设计

GrabInk 使用 C++、Qt 6 和 QML 开发。主界面、区域选择、标注、PNG 保存与剪贴板等核心逻辑由各平台共用，平台差异集中在“如何安全地取得屏幕画面”这一层：
...
这种方式不要求为每个平台重新实现整套编辑器，也避免把 Wayland 支持绑定到某个桌面环境的私有接口。相同的截图一旦交给 GrabInk，后续框选、标注、保存和复制行为保持一致。
...
- Wayland 命令行模式：`wl-clipboard`，用于在 GrabInk 退出后继续保留剪贴板内容
...
```bash
# 区域截图
./build/release/grabink --region

# 窗口截图
./build/release/grabink --window

# 全屏截图
./build/release/grabink --fullscreen

# 延时 3 秒后全屏截图
./build/release/grabink --fullscreen --delay 3
```
...
GrabInk 暂不自行注册全局快捷键。可以在 GNOME 的“设置 → 键盘 → 查看及自定义快捷键 → 自定义快捷键”中绑定以下命令，路径需要替换为可执行文件的实际绝对路径：

```text
/home/user/apps/GrabInk/grabink --region
/home/user/apps/GrabInk/grabink --window
/home/user/apps/GrabInk/grabink --fullscreen
```
...
GrabInk 针对不同环境选择不同的抓屏后端，但复用同一套区域选择和标注流程：
...
GrabInk 以 [GNU General Public License v3.0 or later](LICENSE) 发布。发布修改版或衍生作品时，需要遵守 GPL 的源代码公开和许可证保留要求。Qt 本身由 Qt 项目按其各自许可证提供，不属于 GrabInk 仓库内容。
